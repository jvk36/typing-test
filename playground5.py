...
# ChatGPT Prompt:
# More Problems: a) Reset doesn't clear the screen, b) The highlight appears 
# a few words ahead of the word I type, c) The timer is not stopping after 
# 60 seconds - it is instead starting from 0 again, and d) WPM is not updating 
# as I type. Could you try to fix these issues?
...
class TypingTestApp:
    ...
    def update_words_text(self):
        ...
        end_index = start_index + len(self.words[self.highlighted_word_index])
    ...
    def get_word_start_index(self, index):
        ...
        words_text = " ".join(self.words)
        text = words_text[:words_text.find(self.words[index])].replace(" ", "")
